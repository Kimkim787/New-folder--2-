...
from .forms import ( SeminarForm, SeminarAttendanceForm, TransactionForm, 
                    ...
                    OJTRequirementsForm, StatusWidget,
                    )
...
def admin_signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Successfuly registered admin')
            return redirect('jobplacement:home')  # Change this to your admin dashboard URL
    else:
        form = AdminSignUpForm()
        messages.error(request, 'Failed to registered admin')
...
def ojt_hiring(request):
    ojt_hiring_form = OjtHiringForm() # new ojt company
    ...
    if request.method == 'POST':
        ...
        if allowed_slots >= 1:
            try:
                ...
                # non disclosure
                # download_zipped_ojt_templates(request)
...
@login_required(login_url='jobplacement:admin_login')
def ojt_hiring_info(request, id):
    if not request.user.is_staff:
        messages.info(request, 'Must be staff/admin to access page')
        return redirect('jobplacement:admin_login')
    ...
    if request.method == 'POST':
        ...
        existing_ojt = OJTStudent.objects.filter(student__studID=stud_id).first()
...
def ojt_requirements_submit(request):
    if not StudentUser.objects.get(email = request.user.email):
...
@csrf_exempt
@login_required(login_url='jobplacement:admin_login')
def attend_all_pending(request, id): # primary key sa seminar
    if not request.user.is_staff:
        messages.info(request, 'Must be staff/admin to access page')
        return redirect('jobplacement:admin_login')
...
def non_acad_page(request):
    ...
    def generate_document(template_path, output_path, data):
        try:
            ...
            messages.error(request, "Successfuly generated Docs")
        except:
    ...
    if request.method == 'POST':
        ...
        try: 
            ...
            messages.error(request, "Error identifying witch is which")
    context = {}
...
def view_pdf(request, id):
    ...
    if request.method == 'POST':
        print('POST Method')
        attr_name = request.POST.get('attr_name') # expecting to have a value of 'medical', 'moa'
        ...
        if attr_name == 'nondis':
            ...
            print(f"consent: {pdf.name}")
            print(f"consent: {pdf.url}")
        elif attr_name == 'medical':
            pdf = req.medical
        elif attr_name == 'apl_letter':
    ...
    # context = {'pdf':pdf}
    # return render(request, 'jobplacement/view_pdf.html', context)
